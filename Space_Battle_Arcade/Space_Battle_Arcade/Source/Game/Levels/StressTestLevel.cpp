...
#include "Game/Tools/DebugTools/SAHitboxPicker.h"
...
#include "Game/UI/EditorUI/SAProjectileTweakerWidget.h"
#include "Game/AI/SAShipAIBrain.h"
...
#include "Game/Cameras/SAShipCamera.h"
#include "Game/Components/FighterSpawnComponent.h"
#include "Game/Environment/Nebula.h"

namespace SA
{
	void StressTestLevel::postConstruct()
	{
		...
		{
			modSystem->onActiveModChanging.addWeakObj(sp_this(), &StressTestLevel::handleActiveModChanging);
		}
	...
	void StressTestLevel::startLevel_v()
	{
#if ERROR_CHECK_GL_RELEASE 
	log("StressTestLevel", LogLevel::LOG_WARNING, "ERROR_CHECK_GL_RELEASE enabled. For performance this should be disabled for final release builds.");
#endif
		...
		const sp<UISystem_Editor>& uiSystem= game.getEditorUISystem();
		...
		if (!activeMod)
		{
			log("StressTestLevel", LogLevel::LOG_ERROR, "No active mod");
			return;
		}
		...
		{
			log("StressTestLevel", LogLevel::LOG_ERROR, "Default Spawn Configs not available.");
		}
		...
		if(const sp<PlayerBase>& player = game.getPlayerSystem().getPlayer(0))
		{
			player->getInput().getMouseButtonEvent(GLFW_MOUSE_BUTTON_LEFT).addWeakObj(sp_this(), &StressTestLevel::handleLeftMouseButton);
			player->getInput().getMouseButtonEvent(GLFW_MOUSE_BUTTON_RIGHT).addWeakObj(sp_this(), &StressTestLevel::handleRightMouseButton);
			player->getInput().getKeyEvent(GLFW_KEY_P).addWeakObj(sp_this(), &StressTestLevel::handleDebugCameraRequested);
			player->getInput().getKeyEvent(GLFW_KEY_X).addWeakObj(sp_this(), &StressTestLevel::handleSpectateDetachPressed);
			player->getInput().getKeyEvent(GLFW_KEY_C).addWeakObj(sp_this(), &StressTestLevel::handlePlayerControlTarget);
			player->getInput().getKeyEvent(GLFW_KEY_G).addWeakObj(sp_this(), &StressTestLevel::handleNavigateStarfieldPressed);
		}
	...
	{
		SpaceArcade& game = SpaceArcade::get();
		if (const sp<UISystem_Editor>& uISystem = game.getEditorUISystem())
		{
	...
	void StressTestLevel::handleLeftMouseButton(int state, int modifier_keys)
	{
		if (state == GLFW_PRESS)
		{
			SpaceArcade& game = SpaceArcade::get();
	...
	void StressTestLevel::handleRightMouseButton(int state, int modifier_keys)
	{
		if (state == GLFW_PRESS)
		{
			SpaceArcade& game = SpaceArcade::get();
	...
	void StressTestLevel::handleActiveModChanging(const sp<Mod>& previous, const sp<Mod>& active)
	{
		std::random_device rng;
		std::seed_seq seed{ 28 };
		std::mt19937 rng_eng = std::mt19937(seed);
	...
	void StressTestLevel::handleUIFrameStarted()
	{
	...
	void StressTestLevel::handleSpectateDetachPressed(int state, int modifier_keys, int scancode)
	{
		if (state == GLFW_RELEASE)
		{
	...
	void StressTestLevel::handlePlayerControlTarget(int state, int modifier_keys, int scancode)
	{
	...
	void StressTestLevel::handleNavigateStarfieldPressed(int state, int modifier_keys, int scancode)
	{
		if (state == GLFW_PRESS)
		{
	...
	void StressTestLevel::refreshShipContinuousFireState()
	{
		std::random_device rng;
		std::seed_seq seed{ 7 };
		std::mt19937 rng_eng = std::mt19937(seed);
	...
	void StressTestLevel::refreshProjectiles()
	{
		if (testProjectileConfig)
		{
			for (const sp<Ship>& ship : spawnedShips)
	...
	void StressTestLevel::toggleSpawnTestParticles()
	{
		...
		if (bSpawningParticles)
		{
			if (testParticleConfig)
			{
				...
				if (bContinuousSpawns_ui)
				{
					...
					particleSpawnDelegate->addWeakObj(sp_this(), &StressTestLevel::handleTestParticleSpawn);
	...
	void StressTestLevel::handleTestParticleSpawn()
	{
	...
	void StressTestLevel::onCreateLocalPlanets()
{
	...
	void StressTestLevel::onEntityUnspawned_v(const sp<WorldEntity>& unspawned)
	{
	...
	sp<StarField> StressTestLevel::onCreateStarField()
	{
		if (bEnableStarField)
		{
	...
	void StressTestLevel::render(float dt_sec, const glm::mat4& view, const glm::mat4& projection)
	{
