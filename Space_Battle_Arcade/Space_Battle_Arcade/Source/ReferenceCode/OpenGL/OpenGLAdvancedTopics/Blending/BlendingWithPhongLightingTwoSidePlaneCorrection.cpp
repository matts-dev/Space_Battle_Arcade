...
namespace
{
	...
	const char* frag_shader_src = R"(
				...
				void main(){
					vec4 diffuseSampled = texture(material.diffuseMap, interpTextCoords);

					vec3 ambientLight = light.ambientIntensity * vec3(diffuseSampled);	//diffuse color is generally similar to ambient color
					...
					vec3 diffuseLight = max(dot(toLight, fragNormal), 0) * light.diffuseIntensity * vec3(diffuseSampled);
					...
					fragmentColor = vec4(lightContribution, diffuseSampled.a);
				}
			)";


	const char* plane_vertex_shader_src = R"(
				#version 330 core
				layout (location = 0) in vec3 position;			
				layout (location = 1) in vec3 normal;	
				layout (location = 2) in vec2 textureCoordinates;
				...
				void main(){
					...
					//flip normal if on other side for specular highlighting (going to cause warp divergence :\ )
					//vec3 toView = normalize(cameraPosition - fragPosition); //commented out for conditional fragment shader specular behavior
					//if(dot(fragNormal, toView) < 0){
					//	fragNormal  = fragNormal * -1;
					//}

					interpTextCoords = textureCoordinates;
				}
			)";
	const char* plane_frag_shader_src = R"(
				#version 330 core
				...
				void main(){
					...
					//abs (rather than max) to make regardless of side of plane the light is on cause lighting			
					vec3 diffuseLight = abs(dot(toLight, fragNormal)) * light.diffuseIntensity * vec3(diffuseSampled);

					vec3 toReflection = reflect(-toLight, normal);												//reflect expects vector from light position 
					float specularAmount = 0;
					bool lightSameSideAsCamera = dot(normal, toView) > 0;
					if(lightSameSideAsCamera){
						specularAmount = pow(max(dot(toView, toReflection),0), material.shininess);
					} else {
						//may not be valid glsl to have local varaible within if statements from what I've read; or perhaps their scope isn't respected
						vec3 camToLight = normalize(light.position - cameraPosition);
						vec3 camToFrag = normalize(fragPosition - cameraPosition);
						specularAmount = pow(max(dot(camToLight , camToFrag),0), material.shininess);
					}
					vec3 specularLight = light.specularIntensity* specularAmount * vec3(texture(material.specularMap, interpTextCoords));
	...
	void true_main()
	{
		...
		GLuint windowDiffuseMap = textureLoader("Textures/blending_transparent_window.png", GL_TEXTURE0);
		GLuint windowSpecularMap = textureLoader("Textures/blending_transparent_window_specularmap.png", GL_TEXTURE1);
		//shaders
		...
		Shader glassShader(plane_vertex_shader_src, plane_frag_shader_src, false);
		glassShader.use();
		glassShader.setUniform1i("material.diffuseMap", 0);
		glassShader.setUniform1i("material.specularMap", 1);
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			shader.setUniform1f("light.linear", 0.045f);
			shader.setUniform1f("light.quadratic", 0.0075f);
			...
			for (const auto& pos : windowPositions)
			{
				...
				sortedPositions[glm::length(pos - camera.getPosition())] = pos;
			}

			glassShader.use();
			glassShader.setUniform1i("material.shininess", shininess);
			glassShader.setUniform3f("light.ambientIntensity", ambientColor.x, ambientColor.y, ambientColor.z);
			glassShader.setUniform3f("light.diffuseIntensity", diffuseColor.x, diffuseColor.y, diffuseColor.z);
			glassShader.setUniform3f("light.specularIntensity", lightcolor.x, lightcolor.y, lightcolor.z);
			glassShader.setUniform1f("light.constant", 1);
			glassShader.setUniform1f("light.linear", 0.045f);
			glassShader.setUniform1f("light.quadratic", 0.0075f);
			glassShader.setUniform1i("enableAmbient", toggleAmbient);
			glassShader.setUniform1i("enableDiffuse", toggleDiffuse);
			glassShader.setUniform1i("enableSpecular", toggleSpecular);
			glassShader.setUniform3f("light.position", lightPos.x, lightPos.y, lightPos.z);
			glassShader.setUniform3f("cameraPosition", camPos.x, camPos.y, camPos.z);
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, windowDiffuseMap);
			glActiveTexture(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_2D, windowSpecularMap);
			for (auto iter = sortedPositions.rbegin(); iter != sortedPositions.rend(); ++iter)
			{
				const glm::vec3& pos = iter->second;
				glm::mat4 model;
				model = glm::translate(model, pos);
				glassShader.setUniformMatrix4fv("model", 1, GL_FALSE, glm::value_ptr(model));
				glassShader.setUniformMatrix4fv("view", 1, GL_FALSE, glm::value_ptr(view));
				glassShader.setUniformMatrix4fv("projection", 1, GL_FALSE, glm::value_ptr(projection));
				glBindVertexArray(windowVAO);
