...
namespace
{
	...
	void processInput(GLFWwindow* window)
	{
		...
		{
			//simulates increasing height, which causes fraction width/height to get smaller
			aspect -= 0.05f;
		}
		if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
		{
			//simulates decreasing height, which causes fraction width/height to get larger
			aspect += 0.05f;
		}
	}
	void true_main()
