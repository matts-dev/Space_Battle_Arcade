...
//--------------------------------------------------------------------------------------------
				...
				vec2 parallaxMapping(vec2 texCoords, vec3 toViewFromFrag)
				{
					...
					if(bEnableParallax)
					{
						...
						//this aims to increase parallax effect from steep angles
						//directly above the surface (in tangent space), z will be close to 1.
						//from a steep angle (ie the side), Z will be close to 0.
						//dividing by a small number(ie something close to 0) will result in a large number; increasing the parallax effect.
						// personally, but I think it actually looks worse, 
						//p_uv /= toViewFromFrag.z;   

						//points from p_uv to texCoords (see below for explanation)
						parallaxTexCoords = texCoords - p_uv;

						//for clarity of the underlying system:
						//p_uv points in the direction of the viewer, this is the wrong direction
						//what we want is to start at the texture coordinates, and move towards the coordinates for the given depth
						//the above commented out code is a simplification of this, but I think it gives a false impression of intent of parallax heuristic 
						//parallaxTexCoords = texCoords + -p_uv;
