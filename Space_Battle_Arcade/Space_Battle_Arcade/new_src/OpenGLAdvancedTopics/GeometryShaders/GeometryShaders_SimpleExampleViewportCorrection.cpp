...
namespace
{
	...
	const char* geometry_shader_src = R"(
				...
				void main(){
					...
					gl_Position = gl_in[0].gl_Position + vec4(0, -0.1 * aspect, 0, 0);
					EmitVertex();
					gl_Position = gl_in[0].gl_Position + vec4(0, 0.1 * aspect, 0, 0);
					EmitVertex();
					EndPrimitive();
	...
	void true_main()
	{
		...
		GLuint aspectRatioLoc = glGetUniformLocation(shaderProg, "aspect");
		glUseProgram(shaderProg);
		glUniform1f(aspectRatioLoc, static_cast<float>(width)/height);
