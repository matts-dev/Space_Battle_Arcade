...
namespace
{
	...
	const char* vertex_shader_src = R"(
				...
				void main(){
					...
					fragNormal_TS = TBN_INV * fragNormal_TS;
...
//--------------------------------------------------------------------------------------------
				...
				uniform bool bEnableParallax = true;
				uniform sampler2D parallaxDispDepthMap;
				uniform float parallax_strength_factor = 0.1f;

				vec2 parallaxMapping(vec2 texCoords, vec3 toViewFromFrag)
				{
					vec2 parallaxTexCoords;

					if(bEnableParallax)
					{
						float depth = texture(parallaxDispDepthMap, texCoords).r;
						
						vec2 p_uv = toViewFromFrag.xy;
						p_uv *= depth * parallax_strength_factor;
						//p_uv /= toViewFromFrag.z; 

						//depth is [0, 1] where 0 is closest to the surface
						//const float numLayers = 10;
						const float minLayers = 8.0;
						const float maxLayers = 32.0;
						float numLayers = mix(maxLayers, minLayers, abs(dot(vec3(0.0f,0.0f,1.0f), toViewFromFrag)));

						float layerDepthDelta = 1.0f / 10;
						float currentLayerDepth = 0.0f;

						vec2 delta_p_uv = p_uv / numLayers;

						vec2 currentTexCoords = texCoords;
						float texCoordDepth = texture(parallaxDispDepthMap, currentTexCoords).r;

						//for whatever reason this is confusing to me, because my brain wants to think in heights not depths
						//but since we're dealing with depths, a depth of 1 is deeper than a depth of 0.
						//this loop basically is "while(currentLayer is more shallow than readDepthValue)
						while(currentLayerDepth < texCoordDepth )
						{
							//for clarity of the underlying system:
							//
							// |          eye
							// |         /
							// |--------*--------- mesh true surface; * = fragment pixel for current render
							// |_____  /
							// |solid\_
							// |_______|__________ depth max value
							//
							//p_uv points in the direction of the viewer(eye), this is the wrong (opposite) direction
							//what we want is to start at the texture coordinates(*), and move towards the coordinates for the given depth (not towards camera)
							//thus, the code below I believe is more readable as to the intent of the parallax algorithm
							//
							//currentTexCoords = currentTexCoords + -delta_p_uv;
							//
							//but is simplified to 
							currentTexCoords = currentTexCoords - delta_p_uv;
					
							currentLayerDepth += layerDepthDelta;
							texCoordDepth = texture(parallaxDispDepthMap, currentTexCoords).r;
						}

						//interpolate between the two layers
						float previouslayerDepth = currentLayerDepth - layerDepthDelta;
						vec2 previousTexCoords = currentTexCoords + delta_p_uv; //(move back a layer; the opposite of how we were moving deeper)
						
						//the tutorial doesn't explain this very well, but I'm not too concerned because I think it doesn't actually look great anyways (compared to the simple parallax mapping)
						//
						//When the above loop ends, currentLayerDepth >= texCoordDepth; lets look at the cases of currentLayerDepth > texCoordDepth since that is most common.
						//	 the passedLayerDepth be negative because: value = smallerPositiveNumber - largerPositiveNumber;
						// beforeLayerDepth isn't too sensible either. it takes the depth of the previous sampled tex coord
						//	add adds to it, the layer 2 layers after the associated layer for that text coord? To go back a layer it should be "currentLayerDepth - layerDepthDelta"
						float passedLayerDepth = texCoordDepth - currentLayerDepth;
						float beforeLayerDepth = texture(parallaxDispDepthMap, previousTexCoords).r - currentLayerDepth + layerDepthDelta; 

						float weight = passedLayerDepth / (passedLayerDepth - beforeLayerDepth);
						parallaxTexCoords  = currentTexCoords * weight + previousTexCoords * (1-weight);
						//parallaxTexCoords  = currentTexCoords;

					} else {
						parallaxTexCoords = texCoords;
					}

					return parallaxTexCoords;
				}
...
//--------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------------------------------------------------
					...
					vec2 parallaxTexCoords = parallaxMapping(interpTextCoords, toView);
					if(parallaxTexCoords.x > 1.0f || parallaxTexCoords.y > 1.0f || parallaxTexCoords.x < 0.0f || parallaxTexCoords.y < 0.0f){
						discard; //the edge artificats
					}

					vec3 ambientLight = light.ambientIntensity * vec3(texture(material.diffuseMap, parallaxTexCoords));	//diffuse color is generally similar to ambient color
					vec3 toLight = normalize(lightPos_TS - fragPosition_TS);

					vec3 normal;					
					if(bUseNormalMapping)
					{
						normal = texture(material.normalMap, parallaxTexCoords).rgb;
	...
	//--------------------------------------------------------------------------------------------
					...
					vec3 diffuseLight = max(dot(toLight, normal), 0) * light.diffuseIntensity * vec3(texture(material.diffuseMap, parallaxTexCoords));
					...
					vec3 specularLight = light.specularIntensity* specularAmount * vec3(texture(material.specularMap, parallaxTexCoords));
	...
	bool bUseParallax = true;
	bool bUseBrickTexture = false;
	//----------------------------------
	...
	void processInput(GLFWwindow* window)
	{
		static int initializeOneTimePrintMsg = []() -> int {
			std::cout << "Press F to switch to depth framebuffer" << std::endl
				...
				<< "Press G to change textures" << std::endl
				<< "Press P to toggle parallax" << std::endl
				<< "" << std::endl
				<< "" << std::endl
				<< std::endl;
			return 0;
		...
		{
			bUseBrickTexture = !bUseBrickTexture;
		}
		//--------------------------------------------------
	...
	void true_main()
	{
		...
		//textures
		GLuint diffuseMap = textureLoader("Textures/bricks2.jpg", GL_TEXTURE0);
		GLuint specularMap = textureLoader("Textures/bricks2.jpg", GL_TEXTURE1);
		//--------------------------------------------------------------------------------------------
		stbi_set_flip_vertically_on_load(false);
		GLuint normalMap = textureLoader("Textures/bricks2_normal.jpg", GL_TEXTURE2);
		GLuint displacementMap = textureLoader("Textures/bricks2_disp.jpg", GL_TEXTURE3);

		GLuint toyDiffuse  = textureLoader("Textures/wood.png", GL_TEXTURE0);
		GLuint toySpecular = textureLoader("Textures/wood.png", GL_TEXTURE1);
		GLuint toyNormal  = textureLoader("Textures/toy_box_normal.png", GL_TEXTURE2);
		GLuint toyDisplacement = textureLoader("Textures/toy_box_disp.png", GL_TEXTURE3);
		//--------------------------------------------------------------------------------------------
		...
		shader.setUniform1i("parallaxDispDepthMap", 3);
		...
		//lambda rather than function because maps are local variables to this example file
		auto updateTextures = [&]() {
			if (bUseBrickTexture)
			{
				glActiveTexture(GL_TEXTURE0); glBindTexture(GL_TEXTURE_2D, diffuseMap);
				glActiveTexture(GL_TEXTURE1); glBindTexture(GL_TEXTURE_2D, diffuseMap);
				glActiveTexture(GL_TEXTURE2); glBindTexture(GL_TEXTURE_2D, normalMap);
				glActiveTexture(GL_TEXTURE3); glBindTexture(GL_TEXTURE_2D, displacementMap);
			}
			else
			{
				glActiveTexture(GL_TEXTURE0); glBindTexture(GL_TEXTURE_2D, toyDiffuse);
				glActiveTexture(GL_TEXTURE1); glBindTexture(GL_TEXTURE_2D, toyDiffuse);
				glActiveTexture(GL_TEXTURE2); glBindTexture(GL_TEXTURE_2D, toyNormal);
				glActiveTexture(GL_TEXTURE3); glBindTexture(GL_TEXTURE_2D, toyDisplacement);
			}
		};
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			shader.setUniform1i("bEnableParallax", bUseParallax);
