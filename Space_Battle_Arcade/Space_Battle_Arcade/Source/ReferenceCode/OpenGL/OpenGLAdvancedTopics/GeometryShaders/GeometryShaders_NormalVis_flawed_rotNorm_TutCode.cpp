	...
	const char* frag_shader_src = R"(
			...
			const char* vertex_normalvisualizer_shader_src = R"(
				...
				void main(){
					...
					//vs_out.vertNormal = normalize(vec3(projection * normalViewSpace));
					vs_out.vertNormal = vec3(normalize(projection * normalViewSpace));
				}
			)";
			...
			const char* geometry_normalvisualizer_shader_src = R"(
				...
				void main(){
					...
					for(int i = 0; i < 3; ++i)
					{
						...
						gl_Position = gl_in[i].gl_Position + (vec4(vertices[i].vertNormal, 0) * scale_down);					
						EmitVertex();
