...
namespace
{
	...
	const char* frag_shader_src = R"(
				...
				void main(){
					...
					vec3 emissionLight = vec3(texture(material.emissionMap, interpTextCoords));

					vec3 lightContribution = (ambientLight + diffuseLight + specularLight) + emissionLight;
	...
	void true_main()
	{
		...
		unsigned char *textureData = stbi_load("Textures/container2.png", &img_width, &img_height, &img_nrChannels, 0);
		if (!textureData)
		{
		...
		textureData = stbi_load("Textures/container2_specular.png", &img_width, &img_height, &img_nrChannels, 0);
		if (!textureData)
		{
			std::cerr << "failed to load texture" << std::endl;
			exit(-1);
		...
		textureData = stbi_load("Textures/matrix.jpg", &img_width, &img_height, &img_nrChannels, 0);
		if (!textureData)
		{
			std::cerr << "failed to load texture" << std::endl;
			exit(-1);
		...
		shader.setUniform1i("material.emissionMap", 2);
