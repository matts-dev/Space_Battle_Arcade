...
namespace
{
	...
	const char* vertex_shader_src = R"(
				...
				void main(){
					//below is gouraud shading in view space as opposed to world space

					gl_Position = projection * view * model * vec4(position, 1);

					//handle lighting in view space
					vec3 fragPosition = vec3(view * model * vec4(position, 1));

					//calculate the inverse_tranpose matrix on CPU in real applications; it's a very costly operation
					vec3 fragNormal = mat3(transpose(inverse(view * model))) * rawNormal;
					...
					//diffuse
					vec3 viewLightPos = vec3(view * vec4(lightPosition, 1));
					vec3 toLight = normalize(viewLightPos - fragPosition);
					vec3 normal = normalize(fragNormal);
					vec3 diffuseLight = max(dot(normalize(toLight), normal), 0.0) * diffuseStrength * lightColor;
					...
					//fragLight = clamp(ambientLight + diffuseLight + specularLight), 0, 1) * objectColor;
	...
	const char* frag_shader_src = R"(
				...
				void main(){
					fragmentColor = vec4(fragLight, 1);
				}
			)";
