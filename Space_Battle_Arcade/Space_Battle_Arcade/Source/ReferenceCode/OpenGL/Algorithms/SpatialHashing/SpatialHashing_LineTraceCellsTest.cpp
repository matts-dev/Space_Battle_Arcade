...
namespace
{
	...
	class SpatialHashLineTraceTest final : public IOpenGLDemo
	{
		...
		bool bShowActualCellsInLineTrace = false;
		glm::vec3 axisOffset{ 0, 0.005f, 0 };
		glm::vec3 cachedVelocity;
		Pik::Ray DrawRay;

		SH::SpatialHashGrid<GameEntity> spatialHash{ glm::vec3{1,1,1} };
	...
	public:
		...
		virtual void tickGameLoop(GLFWwindow* window)
		{
			...
			//
			glm::vec3 start = DrawRay.start;
			glm::vec3 end = start + DrawRay.dir * DrawRay.T;

			std::vector<glm::ivec3> lineTraceCells;
			if (bShowActualCellsInLineTrace)
			{
				std::vector<std::shared_ptr<const SH::HashCell<GameEntity>>> outCells;
				spatialHash.lookupCellsForLine(start, end, outCells);
				for (std::shared_ptr<const SH::HashCell<GameEntity>>& cell : outCells)
				{
					lineTraceCells.push_back(cell->location);
				}
			}
			else //show all lookup locations
			{
				spatialHash.findCellLocationsForLine(start, end, lineTraceCells);
			}

			glDepthFunc(GL_ALWAYS);
			SH::drawCells(lineTraceCells, spatialHash.gridCellSize, glm::vec3(0, 0.85f, 0.85f), debugGridShader, glm::mat4(1.0f), view, projection);
			glDepthFunc(GL_LESS);
		...
		void processInput(GLFWwindow* window)
		{
			static int initializeOneTimePrintMsg = []() -> int {
				std::cout << "Hold ALT to move objects with WASD-EQ" << std::endl
					...
					<< " Y to toggle cursor mode, leftclick to shoot ray, U to switch between cell locations and real cells in map" << std::endl
					<< std::endl;
				return 0;
			}();
			...
			{
				bShowActualCellsInLineTrace = !bShowActualCellsInLineTrace;
			}
			...
			if(input.isMouseButtonJustPressed(window, GLFW_MOUSE_BUTTON_LEFT))
			{
				...
				DrawRay = Pik::ObjectPicker::generateRay(
					glm::ivec2(screenWidth, screenHeight), mousePos_TopLeft,
					camera.getPosition(), camera.getUp(), camera.getRight(), camera.getFront(), camera.getFOV(),
					static_cast<float>(screenWidth) / screenHeight);
				Pik::Ray IntersectRay = DrawRay;

				DrawRay.T *= 50;
				//nudge camera slightly so it doesn't appear as an infinitesimal point
				camera.setPosition(camera.getPosition() + glm::vec3(0.0001f));
				IntersectRay.T = 50;
				...
				//	for (const Pik::Triangle& localTri : triangles)
				//	{
				//		Pik::Triangle tri = localTri;
				//		tri.pntA = glm::vec3(model * glm::vec4(localTri.pntA, 1));
				//		tri.pntB = glm::vec3(model * glm::vec4(localTri.pntB, 1));
				//		tri.pntC = glm::vec3(model * glm::vec4(localTri.pntC, 1));

				//		glm::vec3 intersectPnt;
				//		//update ray's "t" if intersection happens
				//		if (IntersectRay.triangleTest(tri, intersectPnt))
				//		{
				//			collisionIdx = idx;
				//		}
				//	}
