...
namespace
{
	...
	const char* frag_shader_src = R"(
				...
				void main(){
					//gl_FragDepth = gl_FragCoord.z //this happens if for early depth testing if no writes occur to gl_FragDepth

					gl_FragDepth = 0.0;			//this write removes early depth testing, depth testing happens after fragment shader is run

					//you don't want to disable early depth testing
					//it saves a lot of calculations within the fragment shader, which is a perf win.
					//so, only write to gl_FragDepth if you know what you're doing and are aware of the potential perf issues. 


					if(gl_FragCoord.x > 400)
						fragmentColor = vec4(1, 0, 0, 1.f);
					else
						fragmentColor = vec4(0, 1, 0, 1.f);
				}
			)";
