...
namespace
{
	...
	const char* vertex_normalvisualizer_shader_src = R"(
				...
				void main(){
					vec4 vertViewSpace = view * model * vec4(position, 1);
					gl_Position = projection * vertViewSpace;

					//calculate the inverse_tranpose matrix on CPU in real applications; it's a very costly operation
					mat3 normalTransform = mat3(transpose(inverse(view * model)));
					vec4 normalViewSpace =  normalize(vec4(normalTransform * normal, 0)); 

					const float scaleDown = 0.1;
					vs_out.normalTailPnt = projection * (vertViewSpace + (normalViewSpace * scaleDown));
				}
			)";
