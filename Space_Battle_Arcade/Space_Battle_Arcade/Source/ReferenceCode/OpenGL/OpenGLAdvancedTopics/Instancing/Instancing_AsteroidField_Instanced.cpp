...
namespace
{
	...
	const char* instanced_vertex_shader_src = R"(
				#version 330 core
				layout (location = 0) in vec3 position;			
				layout (location = 1) in vec3 normal;	
				layout (location = 2) in vec2 textureCoordinates;
	...
	void true_main()
	{
		...
		//set up asteroid positions
		constexpr unsigned int amount = 100000;
		glm::mat4* modelMatrices = new glm::mat4[amount];  //probably should be on heap due to large sizes
		srand(static_cast<int>(glfwGetTime()));
		float radius = 25.0f;
		float offset = 2.5f;
		//following the tutorial's method just for consistency... though it does some questionable stuff.
		for (unsigned int i = 0; i < amount; ++i)
		{
			glm::mat4 model;

			//translate
			float angle = (i / static_cast<float>(amount)) * 360.0f;
			float displacement = (static_cast<int>(rand()) % static_cast<int>(2 * offset * 100)) / (100.0f - offset); //not sure on tutorial logic for displacements -- seems adhoc, but whatever.
			float x = sin(angle) * radius + displacement; //again, wth is the tutorial doing? angle is in degrees. 

			displacement = (static_cast<int>(rand()) % static_cast<int>(2 * offset * 100)) / (100.0f - offset); //not sure on tutorial logic for displacements -- seems adhoc, but whatever.
			float y = displacement * 0.4f;

			displacement = (static_cast<int>(rand()) % static_cast<int>(2 * offset * 100)) / (100.0f - offset); //not sure on tutorial logic for displacements -- seems adhoc, but whatever.
			float z = cos(angle) * radius + displacement; //again, angle is in degrees but tutorial is passing it to call expecting radians
			model = glm::translate(model, glm::vec3(x, y, z));

			//scale ~[0.05,0.24]
			float scale = (static_cast<int>(rand()) % 20) / 100.0f + 0.05f;
			model = glm::scale(model, glm::vec3(scale));

			float rotAngle = static_cast<float>((static_cast<int>(rand()) % 360));
			model = glm::rotate(model, rotAngle, glm::vec3(0.4f, 0.6f, 0.8f)); //again... rotate takes radians. But following tutorial for consistency
		...
		//the largest vertex attrib pointer you can make is vec4, for a mat4 we have to do it column by column
		asteroid.setInstancedModelMatricesData(modelMatrices, amount);
		...
		Shader instancedShader(instanced_vertex_shader_src, frag_shader_src, false);
		...
		*/
		...
		instancedShader.setUniform3f("coneLight.position", camera.getPosition());
		instancedShader.setUniform3f("coneLight.direction", camera.getFront());
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			dirLight.applyUniforms(instancedShader);

			//configure point lights
			pointLights[0].applyUniforms(shader);
			pointLights[0].applyUniforms(instancedShader);
			...
			coneLight.applyUniforms(instancedShader);
			...
			instancedShader.setUniform1i("material.shininess", shininess);
			instancedShader.setUniform3f("light.ambientIntensity", ambientColor.x, ambientColor.y, ambientColor.z);
			instancedShader.setUniform3f("light.diffuseIntensity", diffuseColor.x, diffuseColor.y, diffuseColor.z);
			instancedShader.setUniform3f("light.specularIntensity", lightcolor.x, lightcolor.y, lightcolor.z);
			//instancedShader.setUniform3f("light.specularIntensity", specularStrength, specularStrength, specularStrength);
			instancedShader.setUniform1i("enableAmbient", toggleAmbient);
			instancedShader.setUniform1i("enableDiffuse", toggleDiffuse);
			instancedShader.setUniform1i("enableSpecular", toggleSpecular);
			instancedShader.setUniform1i("toggleFlashLight", toggleFlashLight);
			instancedShader.setUniform3f("cameraPosition", camPos.x, camPos.y, camPos.z);
			...
			{
				instancedShader.use();
				instancedShader.setUniformMatrix4fv("view", 1, GL_FALSE, glm::value_ptr(view));  
				instancedShader.setUniformMatrix4fv("projection", 1, GL_FALSE, glm::value_ptr(projection));
				//Asteroid.draw(shader);
				asteroid.drawInstanced(instancedShader, amount);
			}
