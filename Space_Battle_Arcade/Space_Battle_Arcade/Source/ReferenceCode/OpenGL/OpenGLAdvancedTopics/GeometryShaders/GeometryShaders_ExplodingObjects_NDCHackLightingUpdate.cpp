...
namespace
{
	...
	const char* geometry_shader_src = R"(
				...
				void main(){
					...
					vec3 faceNormal = normalize(cross(b - a, c - a)); //implode
					//vec3 faceNormal = normalize(cross(c - a, b - a)); //explode

					float offsetScalar = (sin(timeSecs) +1.0f) / 2.0f;
					offsetScalar *= MAGNITUDE;
					vec4 offset = vec4(faceNormal * offsetScalar, 1.0f);
					

					gl_Position =		   gl_in[0].gl_Position + offset;		//this one is easy to forget
					fragNormal =		vertices[0].fragNormal;
					fragPosition =		vertices[0].fragPosition + vec3(offset); //techinically this offset wrong (is in NDC), so we need to hit it with the inverse transform
					interpTextCoords =	vertices[0].interpTextCoords;
					EmitVertex();

					gl_Position =		   gl_in[1].gl_Position + offset;		
					fragNormal =		vertices[1].fragNormal;
					fragPosition =		vertices[1].fragPosition + vec3(offset); //techinically this offset wrong (is in NDC), so we need to hit it with the inverse transform
					interpTextCoords =	vertices[1].interpTextCoords;
					EmitVertex();

					gl_Position =		   gl_in[2].gl_Position + offset;		
					fragNormal =		vertices[2].fragNormal;
					fragPosition =		vertices[2].fragPosition + vec3(offset); //techinically this offset wrong (is in NDC), so we need to hit it with the inverse transform
					interpTextCoords =	vertices[2].interpTextCoords;
					EmitVertex();
	...
	void true_main()
	{
		...
		*/
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			//draw objects
			shader.use();
			shader.setUniform1f("timeSecs", static_cast<float>(glfwGetTime()));
