...
#include "ModelWithNormalMaps/Model_NM.h"
...
namespace
{
	...
	//--------------------------------------------------------------------------------------------
				...
				uniform bool bDisplayNormals = false;
				...
				uniform bool bUseNormalMapping = false; 
	...
	//--------------------------------------------------------------------------------------------
					...
					{
						//normal = texture(material.texture_normalmap0, interpTextCoords).rgb;	//should normalize what is read from the texture map... which sucks because this is definite
						normal = normalize(texture(material.texture_normalmap0, interpTextCoords).rgb); //unfortunately have to normalize what is stored in texture map, which is probably a perf hit if done in real applications
	...
	//--------------------------------------------------------------------------------------------
	//helper class to get lights moving
	class PointLightWithAccessors : public PointLight
	{
	public: //accessors
		PointLightWithAccessors(
		...
		const std::string& getGLSLPositionStr()
		{
			return glslPositionStr;
		}
	};
	//--------------------------------------------------------------------------------------------
	//--------------------------------
	...
	float spinSpeed = 25.0f; //factor for how fast lights spin
	...
	void processInput(GLFWwindow* window)
	{
		static int initializeOneTimePrintMsg = []() -> int {
			std::cout << "Press F to switch to depth framebuffer" << std::endl
				...
				<< "Press M to display normals" << std::endl
				<< "Press O/P to increase/decrease rotation speed" << std::endl
				<< std::endl;
			return 0;
		}();
		...
		if (input.isKeyDown(window, GLFW_KEY_P))
		{
			spinSpeed += 10.0f * deltaTime;
		}
		...
		{
			bDisplayNormals = !bDisplayNormals;
		}
	...
	void true_main()
	{
		...
		Model_NM meshModel("Models/nanosuit/nanosuit.obj");
		...
		*/
		...
		PointLightWithAccessors pointLights[] = {
			{
				pntAmbient,
				pntDiffuse,
				pntSpecular,
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			//draw light
			static std::vector<glm::vec3> lightCorrectedPositions(sizeof(pointLights) / sizeof(PointLight));
			...
			for (size_t i = 0; i < sizeof(pointLights) / sizeof(PointLight); ++i)
			{
				...
				glm::vec3 lightStart = pointLights[i].getPosition();
				model = glm::rotate(model, glm::radians(rotateLight ? spinSpeed * currentTime : 0), glm::vec3(0, 1.f, 0)); //apply rotation leftmost (after translation) to give it an orbit
				model = glm::translate(model, lightStart);
				model = glm::scale(model, glm::vec3(0.2f));
				glm::vec3 lightPos = glm::vec3(model * glm::vec4(0, 0, 0, 1));
				lightCorrectedPositions[i] = lightPos;
			...
			for (size_t i = 0; i < sizeof(pointLights) / sizeof(PointLight); ++i)
			{
				pointLights[i].applyUniforms(shader);
				shader.setUniform3f(pointLights[i].getGLSLPositionStr().c_str(), lightCorrectedPositions[i]);
			}
			...
			shader.setUniform1i("bDisplayNormals", bDisplayNormals);
			//--------------------------------------------------------------------------------------------
