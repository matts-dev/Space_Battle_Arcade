...
namespace
{
	...
	const char* frag_shader_src = R"(
				...
				void main(){
					...
					//float D;float G;vec3 F; //defining outside of loop for debug visualization
					...
					for(int light_idx = 0; light_idx < NUM_LIGHTS && light_idx < numLightsToRender; ++light_idx){
						...
						//debug visualizes
						debug = vec4(albedo/PI, 1.0f);
						//debug = vec4(vec3(roughness), 1.0f);
						//debug = vec4(vec3(metalness), 1.0f);
						//debug = vec4(zeroIncidence, 1.0f);
						//debug = vec4(vec3(D), 1.0f);
						//debug = vec4(vec3(G), 1.0f);
						//debug = vec4(F, 1.0f);
						//debug = vec4(kDiffuse,1.0f);
					}							
					...
					//vec3 toLight = normalize(lightPosition - fragPosition_WS);
					//vec3 diffuseLight = max(dot(toLight, fragNormal_WS), 0) * lightColor;
	...
	void true_main()
	{
		...
		shader.setUniform1i("bUseTextureData", true);
