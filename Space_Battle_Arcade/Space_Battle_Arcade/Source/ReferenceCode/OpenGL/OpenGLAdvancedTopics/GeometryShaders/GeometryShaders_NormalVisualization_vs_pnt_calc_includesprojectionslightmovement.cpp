...
namespace
{
	...
	const char* vertex_normalvisualizer_shader_src = R"(
				...
				void main(){
					...
					//mat3 normalTransform = mat3(view * transpose(inverse( model)));
					vec4 normalViewSpace =  normalize(projection * vec4(normalTransform * normal, 0)); 

					const float scaleDown = 0.1;
					vs_out.normalTailPnt = gl_Position + (normalViewSpace * scaleDown);
				}
			)";
	...
	const char* geometry_normalvisualizer_shader_src = R"(
				...
				void main(){
					...
					for(int i = 0; i < 3; ++i)
					{
						...
						gl_Position = vertices[i].normalTailPnt;					
						EmitVertex();
