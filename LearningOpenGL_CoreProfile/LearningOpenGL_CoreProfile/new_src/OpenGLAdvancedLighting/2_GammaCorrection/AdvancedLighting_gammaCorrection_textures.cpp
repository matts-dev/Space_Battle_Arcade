...
namespace
{
	...
	const char* frag_shader_src = R"(
				...
				uniform bool bEnableSquareAttenuation = false;
				uniform bool bEnableBlinnPhong = false;
				...
				void main(){
					...
					int SPEC_SHININESS = material.shininess;
					vec3 toView = normalize(cameraPosition - fragPosition);
					float specularAmount;
					vec3 specularLight;
					if(!bEnableBlinnPhong){
						...
						specularAmount = pow(max(dot(toView, toReflection), 0), SPEC_SHININESS);
						specularLight = light.specularIntensity* specularAmount * vec3(texture(material.specularMap, interpTextCoords));
					} else {
						vec3 averageVec = normalize(toView + toLight);
						specularAmount = pow(max(dot(normal, averageVec), 0), SPEC_SHININESS * 4);
						specularLight = light.specularIntensity* specularAmount * vec3(texture(material.specularMap, interpTextCoords));
					}
					...
					if(bEnableGammaCorrection)
					{
						...
						fragmentColor.rgb = pow(fragmentColor.rgb,  vec3(1/gammaValue));
					}
				}
			)";
	...
	bool enableSquareAttentuation = true;
	...
	void processInput(GLFWwindow* window)
	{
		...
		{
			enableBlinnPhong = !enableBlinnPhong;
		}
		if (input.isKeyJustPressed(window, GLFW_KEY_T))
		{
			enableSquareAttentuation = !enableSquareAttentuation;
		}
	...
	void true_main()
	{
		...
		unsigned char* textureData = stbi_load("Textures/woodfloor.png", &img_width, &img_height, &img_nrChannels, 0);
		if (!textureData)
		{
		...
		//--------------------------------------------------------
		//when textures were designed in screen space, we need to mark them as such with GL_SRGB (and I believe GL_SRGB_ALPHA corresponds to GL_RGBA)
		//glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB, img_width, img_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, textureData);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB_ALPHA, img_width, img_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, textureData);
		//--------------------------------------------------------
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			shader.setUniform1i("bEnableSquareAttenuation", enableSquareAttentuation);
			shader.setUniform1i("bEnableBlinnPhong", enableBlinnPhong);
			//--------------------------------------------------------------------------------------------------------------------------
