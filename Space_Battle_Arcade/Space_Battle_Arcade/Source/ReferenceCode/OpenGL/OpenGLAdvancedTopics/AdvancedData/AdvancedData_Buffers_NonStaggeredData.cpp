...
namespace
{
	...
	void true_main()
	{
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(verts) + sizeof(colors), nullptr, GL_DYNAMIC_DRAW); //null just allocates the memory without actually filling anything into them memory
		...
		while (!glfwWindowShouldClose(window))
		{
			processInput(window);
			//-------------------------------------------------- advanced data ---------------------------------------------------------------------
			size_t activeValueOffset;
			size_t skipPositions = 0;
			if (activeValue < Modify::RED)
			{
				activeValueOffset = activeValue;
			}
			else
			{
				activeValueOffset = activeValue - Modify::RED;
				skipPositions = sizeof(verts);
			}
			size_t offset = (vertexIndex * 3 * sizeof(float)) + activeValueOffset * sizeof(float) + skipPositions;
			...
			if (colorDirty)
			{
				//this doesn't have to be a single float, that is just what I used in my example; it could be an array 
				glBufferSubData(GL_ARRAY_BUFFER, offset, sizeof(targetValue), &targetValue);
