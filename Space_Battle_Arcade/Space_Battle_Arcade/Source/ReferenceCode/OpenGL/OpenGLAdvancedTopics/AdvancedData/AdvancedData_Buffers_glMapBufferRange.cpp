...
namespace
{
	...
	void true_main()
	{
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			{
				//char* ptr = reinterpret_cast<char*>(glMapBufferRange(GL_ARRAY_BUFFER, 0, vertsize*6, GL_MAP_READ_BIT));
				//assert(ptr != nullptr);
				//ptr += offset;

				//length specifies how much to copy from the start, not the length of the buffer
				char* ptr = reinterpret_cast<char*>(glMapBufferRange(GL_ARRAY_BUFFER, offset, vertsize * 6 - offset, GL_MAP_READ_BIT));
				assert(ptr != nullptr);
			...
			if (colorDirty)
			{
				//char* ptr = reinterpret_cast<char*>(glMapBufferRange(GL_ARRAY_BUFFER, 0, vertsize * 6, GL_MAP_WRITE_BIT));
				//assert(ptr != nullptr);
				//ptr += offset;

				//length specifies how much to copy from the start, not the length of the buffer
				char* ptr = reinterpret_cast<char*>(glMapBufferRange(GL_ARRAY_BUFFER, offset, vertsize * 6 - offset, GL_MAP_WRITE_BIT));
				assert(ptr != nullptr);
