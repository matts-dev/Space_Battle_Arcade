...
namespace
{
	...
	const char* vertex_normalvisualizer_shader_src = R"(
				...
				void main(){
					...
					//calculate the inverse_tranpose matrix on CPU in real applications; it's a very costly operation
					mat3 normalTransform = mat3(transpose(inverse(view * model)));
					vec4 normalViewSpace =  vec4(normalTransform * normal, 0); 
					vs_out.vertNormal = normalize(projection * normalViewSpace);
					vs_out.w = vs_out.vertNormal.w;
					//vs_out.vertNormal = vec4(vec3(normalize(projection * normalViewSpace)),0); //why does this cause an issue? it appears something is in the w coordinate
					
					//vs_out.vertNormal = vec4(vs_out.vertNormal.x, vs_out.vertNormal.y, vs_out.vertNormal.z, 0); //doesn't work, something is in the w component
					//vs_out.vertNormal = vec4(vs_out.vertNormal.x, vs_out.vertNormal.y, vs_out.vertNormal.z, vs_out.vertNormal.w); //works
				}
			)";
	...
	const char* geometry_normalvisualizer_shader_src = R"(
				...
				void main(){
					...
					for(int i = 0; i < 3; ++i)
					{
						...
						gl_Position = gl_in[i].gl_Position + vertices[i].vertNormal * scale_down;					
						EmitVertex();
	...
	const char* frag_normalvisualizer_shader_src = R"(
				...
				void main(){
					...
					//fragmentColor = vec4(wColor, wColor, wColor, 1);
				}
			)";
